...
class FileTransfer:
    ...
    def check_folders(self, folder_src, folder_dst):
        ...
        for path_to_check in [folder_src, folder_dst]:
            ...
            elif not os.path.exists(folder_src):
    ...
    def compare(self):
        ...
        try:
            self.src_dir_list = Tools.get_all_dirs_from_path(self.base_folder_src, mode="base_directory")
            self.dst_dir_list = Tools.get_all_dirs_from_path(self.base_folder_dst, mode="base_directory")
            self.base_folder_src = os.path.dirname(self.base_folder_src)
            self.base_folder_dst = os.path.dirname(self.base_folder_dst)
            self.cmp_manager.write_compare_file(self.create_entities)
        except OSError as os_error:
            err_text = f"errore durante la lettura del percorso {os_error.filename}: {os.strerror(os_error.errno)}"
            logging.error("[%-15s]: errore - %s", "FileTransfer", err_text)
            raise PathError(error_text="errore, consulta il log per ulteriori dettagli")
        except ComparingFoldersError as comparing_error:    # scrivo su log e propago
            logging.error("[%-15s]: errore - %s", "FileTransfer", comparing_error.error_text)
            raise ComparingFoldersError(error_text="errore, consulta il log per ulteriori dettagli")
        else:
    ...
    def create_entities(self):
        ...
        index_src = 0
        index_dst = 0
        print(self.src_dir_list)
        print(self.dst_dir_list)
        print(self.base_folder_src)
        print(self.base_folder_dst)
        try:
...
class CompareFileParser:
    ...
    def write_compare_file(self, line_generator):
        ...
        self.compare_file = CompareFile(COMPARE_FILE_PATH)       # creo un nuovo file
...
class CompareFile:
    ...
    def __init__(self, compare_file_path):
        ...
        while True:     # creo il file object per eseguire operazioni r/w
            try:
                test_path = os.path.normpath(os.path.join(os.getcwd(), self.compare_file_path, self.compare_file_name))
